...
it("returns documents with every matching tag", async (ctx) => {
	...
	const { result } = renderHook(
		() => useAllPrismicDocumentsByUIDs(type, uids),
		{
...
it("supports params", async (ctx) => {
	...
	const { result } = renderHook(
		() => useAllPrismicDocumentsByUIDs(type, uids, params),
		{
			wrapper: createWrapper(client),
		},
	);
...
it("supports explicit client", async (ctx) => {
	...
	const { result } = renderHook(() =>
		useAllPrismicDocumentsByUIDs(type, uids, { client }),
	);
...
it("returns failed state on error", async (ctx) => {
	...
	const { result } = renderHook(
		() => useAllPrismicDocumentsByUIDs("type", ["uid"]),
		{
			wrapper: createWrapper(client),
		},
	);
